...
const NetworkAsciiArt = ({ isActive, transitionTo, transitionFrom, transitionDirection }) => {
    ...
    nodes.forEach((node, idx) => {
      ...
      node.connections.forEach(connIdx => {
        if (connIdx > idx) { // Avoid duplicates
          ...
          drawConnection(newGrid, node.x, node.y, target.x, target.y, connectionChars);
        }
      });
    });
  ...
  useEffect(() => {
    if (transitionTo === "3d" && isActive) {
      ...
      const animate = () => {
        ...
        for (let y = 0; y < height; y++) {
          for (let x = 0; x < width; x++) {
            if (transitionalGrid[y][x] !== ' ' && Math.random() < replaceThreshold) {
              // Replace with crazy char
              transitionalGrid[y][x] = crazyChars[Math.floor(Math.random() * crazyChars.length)];
            }
          }
        }
      ...
      for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
          if (startGrid[y][x] !== ' ' && Math.random() < 0.5) {
            startGrid[y][x] = crazyChars[Math.floor(Math.random() * crazyChars.length)];
          }
        }
      }
      setGrid(startGrid);
      ...
      const animate = () => {
        ...
        for (let y = 0; y < height; y++) {
          for (let x = 0; x < width; x++) {
            if (transitionalGrid[y][x] !== ' ' && crazyChars.includes(transitionalGrid[y][x]) && Math.random() < replaceThreshold) {
              // Replace with network char
              const possibleChars = [...nodeChars, ...connectionChars];
              transitionalGrid[y][x] = possibleChars[Math.floor(Math.random() * possibleChars.length)];
            }
          }
        }
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        if (grid[y][x] !== ' ' && Math.random() < 0.1) {
          ...
          if (Math.random() < 0.2) {
            newGrid[y][x] = brightChars[Math.floor(Math.random() * brightChars.length)];
          } 
          // Or revert to normal connection character
          else if (Math.random() < 0.3 && connectionChars.includes(grid[y][x])) {
            newGrid[y][x] = connectionChars[Math.floor(Math.random() * connectionChars.length)];
          }
        }
      }
    }
  ...
  const getContainerClasses = () => {
    let classes = 'flex justify-center items-center min-h-screen bg-black p-4 transition-opacity duration-1000 ';
            ...
            <div key={y} className="char-transition">
              {row.map((char, x) => (
                <span 
                  ...
                  style={{
                    transition: isTransitioning ? 'color 300ms ease-out' : 'none'
                  }}
                >
                  {char}
                </span>
