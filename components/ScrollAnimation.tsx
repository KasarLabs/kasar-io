...
const ScrollAnimation: React.FC<ScrollAnimationProps> = ({ onScrollComplete }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const relativeScroll = Math.max(0, Math.min(1, (scrollPosition - containerTop) / scrollHeight));
      
      // Calcul de l'index du mot actif basé sur la position de défilement
      const wordIndex = Math.min(Math.floor(relativeScroll * words.length), words.length - 1);
